...
class ConvolutionalProcessingBlockBNRC(nn.Module):
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=self.num_filters, out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        out = self.layer_dict['conv_1'].forward(out)
...
class ConvolutionalProcessingBlockRC(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockRC, self).__init__()
